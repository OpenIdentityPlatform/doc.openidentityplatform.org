...
escapeJson(string)
----
Returns a JSON escaped string, applying JSON string escaping rules to the provided value.
Useful when embedding dynamic values inside a JSON structure within an expression.
.Parameters
--

string::
The string to escape, which may be `null`.
...
string::
The JSON escaped string, or `null` if the input was `null`.
