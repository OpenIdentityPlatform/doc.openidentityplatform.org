...
Severity: ERROR
+
Message: Could not give change %s of domain "%s" back to the replication server after the replay which owned it was unwound: %s. The change has been released without its failure being counted, and the session is being restarted so that the change is delivered again.
